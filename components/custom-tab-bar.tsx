...
export default function CustomTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const onScanPress = async () => {
    ...
    if (!result.canceled) {
      ...
      if (uri) {
        await AsyncStorage.setItem('nutra:pendingImageUri', uri);
        go('meals');
      } else {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        ...
        <TabItem name="meals" icon="restaurant" label="REFEIÇÕES" />

        {/* Central Button Space */}
        <View style={styles.centerSpace} />

        <TabItem name="recipes" icon="menu-book" label="RECEITAS" />
        <TabItem name="settings" icon="person" label="PERFIL" />
      </BlurView>
